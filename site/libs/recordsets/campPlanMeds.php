 ...
 */
require_once("objects/campPlanMed.php");
...
class CampPlanMeds {
    ...
    function traerPorCamp($campId) {
        $conn = new ConexionBD(MOTORBD, SERVIDORBD, BASEDATOS, USUARIOBD, CLAVEBD);
        if ($campId != 0) {
            if ($conn->conectar()) {
                $conn->consulta("SET NAMES 'utf8'");            
                $sql = "SELECT * FROM `CampPlanMed`  WHERE campId = :campId";
    ...
    private function traerFila($fila) {
        $resultado = NULL;
        if ($fila) {            
            $resultado = new CampPlanMed($fila['plaId'], $fila['campId'], $fila['plaUbicacion'], $fila['plaMedio'], $fila['plaDispositivo'], $fila['plaFormato'], $fila['plaIniCamp'], $fila['plaFinCamp'], $fila['plaAdServ'], $fila['plaInversion'], $fila['plaKPI'], $fila['plaObjetivo'], $fila['plaFecCre'], $fila['plaIp'], $fila['plaConjAnFB']);
        }
        return $resultado;
    }
    ...
    function cantidad($filterBy = '') {
        ...
        if ($conn->conectar()) {
            $conn->consulta("SET NAMES 'utf8'");            
            $sql = "SELECT count(*) cantidad FROM `CampPlanMed`";
            if (!empty($filterBy)) {
                $sql .= " WHERE " . $filterBy;
            }
    ...
    function borrarTodo($campId) {
        $conn = new ConexionBD(MOTORBD, SERVIDORBD, BASEDATOS, USUARIOBD, CLAVEBD);
        if($campId!=""){
            if ($conn->conectar()) {
                $conn->consulta("SET NAMES 'utf8'");            
                $sql = "DELETE FROM `CampPlanMed`";
                $sql .= " WHERE campId = :campId";
